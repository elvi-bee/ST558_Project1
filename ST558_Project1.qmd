...
summary.census <- function(census_tibble,
  ...
  numvar_options <- c("AGEP", "GASP", "GRPIP", "JWAP", "JWDP", "JWMNP")
  if(!all(numvars %in% numvar_options))
    stop("Please enter one or more valid numeric variables, or enter no numeric variables to accept default. Default is all numeric variables.")
  ...
  for(i in 1:length(numvars)) {
    if (numvars[i] == "AGEP") {
      mean_vector[i] <- sum(census_tibble$AGEP*census_tibble$PWGTP)/sum(census_tibble$PWGTP)
      names(mean_vector)[i] <- "AGEP"
      sd_vector[i] <- sqrt(sum(census_tibble$AGEP**2*census_tibble$PWGTP)/sum(census_tibble$PWGTP)-mean_vector[i]**2)
      names(sd_vector)[i] <- "AGEP"
    }
  }
  for(i in 1:length(numvars)) {
    if (numvars[i] == "GASP") {
      mean_vector[i] <- sum(census_tibble$GASP*census_tibble$PWGTP)/sum(census_tibble$PWGTP)
      names(mean_vector)[i] <- "GASP"
      sd_vector[i] <- sqrt(sum(census_tibble$GASP**2*census_tibble$PWGTP)/sum(census_tibble$PWGTP)-mean_vector[i]**2)
      names(sd_vector)[i] <- "GASP"
    }
  }
  for(i in 1:length(numvars)) {
    if (numvars[i] == "GRPIP") {
      mean_vector[i] <- sum(census_tibble$GRPIP*census_tibble$PWGTP)/sum(census_tibble$PWGTP)
      names(mean_vector)[i] <- "GRPIP"
      sd_vector[i] <- sqrt(sum(census_tibble$GRPIP**2*census_tibble$PWGTP)/sum(census_tibble$PWGTP)-mean_vector[i]**2)
      names(sd_vector)[i] <- "GRPIP"
    }
  }
  for(i in 1:length(numvars)) {
    if (numvars[i] == "JWMNP") {
      mean_vector[i] <- sum(census_tibble$JWMNP*census_tibble$PWGTP)/sum(census_tibble$PWGTP)
      names(mean_vector)[i] <- "JWMNP"
      sd_vector[i] <- sqrt(sum(census_tibble$JWMNP**2*census_tibble$PWGTP)/sum(census_tibble$PWGTP)-mean_vector[i]**2)
      names(sd_vector)[i] <- "JWMNP"
    }
  }
  list("Means" = mean_vector, "StandardDeviations" = sd_vector)
}   
```
...
```{r}
summary.census(data_to_plot, numvars = c("AGEP","GASP","GRPIP","JWMNP"))
```
...
#tbl <- as_tibble(census_tibble) #filter doesn't seem to like census class

#  get(numvar)
#  get(catvar)
  
# filter numvars
#AGEP_filtered <- if (numvar == "AGEP") filter(tbl, AGEP >= 1 & AGEP <= 99)
...
#filter catvars
#FER_filtered <- if (catvar == "FER") filter(census_tibble, as.integer(FER) %in% c(2,3))
...
```{r}
# Convert times to mid-point times

view(out2)
timevars <- out2 |>
  select(JWAP, JWAP_label, JWDP, JWDP_label)

#stringr::str_split("9:40 p.m. to 9:44 p.m.", " to ")
...
```{r}
# duplicate - can delete this later
#add time conversion into main code block
...
# two lines below for time parsing - time vars categorized as categorical in order to map the codes over from the json
