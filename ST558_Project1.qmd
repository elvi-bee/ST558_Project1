...
Starting on the multi-year API selection. This still needs a lot of work.

This function should call your single year function as many times as needed and then combine
the data into one final tibble (a year variable should be included in this final tibble)
# user input can be multiple years, e.g. 2015, 2016, 2017
# run API builder for each year above
# combine all output data for various years / various API calls
# add year column
...
# inputs: years, numvars = "AGEP", catvars = "SEX", geo_level = "state", geo_subset
MultiYear_API_Builder <- function(years, numvars = "AGEP", catvars = "SEX", geo_level = "state", geo_subset){
  ...
  for (y in year) {
    ...
    API_URL_Builder(year, numvars, catvars, geo_level, geo_subset)
    # return each iteration of loop and save into a tibble, add year of data, repeat (repeating with each loop iteration)
    }
  
  
  # output all as a single tibble that will also include a year column
}
