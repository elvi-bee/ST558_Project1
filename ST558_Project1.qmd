...
```{r}
#Starting work on converting times to middle times
...
midtimevars <- timevars |>
  mutate(
    # grab everything before to
    start_str  = sub("\\s+to.*$", "", JWAP_label),
    # turn a.m./p.m. into AM/PM bc this caused an error earlier
    start_str  = str_replace_all(start_str, c("a.m." = "AM", "p.m." = "PM")),
    start_time = parse_date_time(start_str, orders = "I:M p", quiet = TRUE),
    mid_time   = start_time + minutes(2),
    mid_hhmm  = format(mid_time, "%I:%M %p"))
